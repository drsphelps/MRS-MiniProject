...
def step_along(start_n, end_n, grid):
  centre, _ = find_circle(start_n, end_n)

  start = start_n.position - centre
  end = end_n.position - centre

  clockwise = True if end_n.pose[YAW] - start_n.pose[YAW] < 0 else False
  angle = angle_between(start, end)

  for a in np.linspace(0, angle, 100):
    a = -a if clockwise else a
    vec = rotate(start, a) + centre

    if not grid.is_free(vec):
      return True
...
def adjust_pose(node, final_position, occupancy_grid):
  ...
  direction = final_position - node.position
  ...
  if step_along(node, final_node, occupancy_grid):
    print("REJECT")
    return None
  return final_node
