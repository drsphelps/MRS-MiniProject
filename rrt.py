...
def adjust_pose(node, final_position, occupancy_grid):
  ...
  def collision(vector, origin, arc_angle, clockwise):
    # Walk along the arc
    for angle in np.linspace(start=0., stop=arc_angle, num=100):
      if clockwise:
        angle = -angle
      if not occupancy_grid.is_free(rotate(vector, angle) + origin):
        return True
    return False
  ...
  # Let vA = initial position, vB = final position, vC = position of center of circle
  vA = node.position
  vB = final_position
  vBA = vB - vA

  alpha = np.arctan2(vBA[Y], vBA[X])
  final_node.pose[YAW] = 2 * alpha - node.pose[YAW]

  vC, radius = find_circle(node, final_node)
  vAC = vA - vC
  vBC = vB - vC
  arc_angle = angle_between(vBC, vAC)
  thetaA = node.pose[YAW]
  thetaB = final_pose[YAW]

  if collision(vAC, vC, arc_angle, (thetaA > thetaB)):
    return None
...
def rrt(start_pose, goal_position, occupancy_grid):
  ...
  for _ in range(MAX_ITERATIONS): 
    ...
    if v is None:
      # There is no valid arc to the closest node (have found collision), so go back to the beginning and sample
