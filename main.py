...
class LeaderLaser(object):
    def __init__(self, name):
        ...
        self._clusters_publishers = [
            rospy.Publisher('/t0/cluster0', Marker, queue_size=5),
            rospy.Publisher('/t0/cluster1', Marker, queue_size=5),
            rospy.Publisher('/t0/cluster2', Marker, queue_size=5),
            rospy.Publisher('/t0/cluster3', Marker, queue_size=5),
            rospy.Publisher('/t0/cluster4', Marker, queue_size=5)]
        # For publishing the centroid
        self._centroid_publisher = rospy.Publisher('/t0/centroid', Marker, queue_size=1)
    ...
    def callback(self, msg):
        ...
        for i, d in enumerate(msg.ranges):
            ...
            if not np.isnan(d) and not np.isinf(d) and _within(angle, self._min_angle, self._max_angle) and d < 2.:
